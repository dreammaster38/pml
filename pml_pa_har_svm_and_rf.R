...
#########################################################
# optional: Train a Polynomial SVM without special tuning
#########################################################
modFitSvm <- train(training$classe ~.,
                   do.trace=100,
                   data = training,
                   method = "svmPoly",
                   trControl = ctrl,
...
# show OOB
print(modFitSvm)
print(modFitSvm$finalModel)

# predict against our cross validation set created in step 2 to find out the accuracy of our model.
predCrossSvm <- predict(modFitSvm, cross)
print(confusionMatrix(predCrossSvm, cross$classe))

# prediction results on the supplied test set for PolySVM
predSvm <- predict(modFitSvm, testing)
print(predSvm)

# Print the overall agreement and Kappa:
accuracySummarySvm <- postResample(predCrossSvm, cross$classe)
print(accuracySummarySvm)

# compare PolySVM with Random FOrest
resamps <- resamples(list(SVM_Polynomial = modFitSvm, Random_Forest = modFitRf))
summary(resamps)

# create a BOX-Plot to show comparison
